...
class Variables
{
    ...
    int Create_new(int n, int j, int temp_N, Variables *scopes, int scopes_amount){
        ...
        int empties2 = 0;
        int i = 0;
        int temp_scopes_amount = scopes_amount;
              for(int m = 0; temp_N + n > g_N; m++){
                  if(temp_scopes_amount - j > 0 ){
                      for (i = j + empties1; i <= temp_scopes_amount; i++){
                          if( !scopes[i].amongus.empty()){
                            char *temp_point;
                            temp_point = scopes[i].amongus.back();          
                            temp_point = nullptr;
                            scopes[i].amongus.pop_back();
                            old =  scopes[i].amonguscapacity.back(); //тут я храню размер переменной, которую удаляю
